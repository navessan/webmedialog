...
function db_connect_real($host, $user, $pass, $db_name, $db_type, $port, $retries = 1) {
	global $cnn_id;
	...
	die("FATAL: Cannot connect to SQL server on '$host'. Please make sure you have specified a valid SQL database name in 'include/config.php'\n");
...
function db_execute($sql, $log = TRUE, $db_conn = FALSE) {
	...
	if (read_config_option("log_verbosity") == VERBOSITY_DEVDBG) {
		app_log("DEVEL: SQL Exec: \"" . $sql . "\"", FALSE);
	}
	...
	while (1) {
		...
		if (($log) || (read_config_option("log_verbosity") == VERBOSITY_DEBUG)) 
		{
			if ((substr_count($db_conn->ErrorMsg(), "Deadlock")) || ($db_conn->ErrorNo() == 1213) || ($db_conn->ErrorNo() == 1205)) {
				$errors++;
				if ($errors > 30) {
					app_log("ERROR: Too many Lock/Deadlock errors occurred! SQL:'" . str_replace("\n", "", str_replace("\r", "", str_replace("\t", " ", $sql))) ."'", TRUE);
					return(0);
				}else{
					usleep(500000);
					continue;
			...
			{
				app_log("ERROR: A DB Exec Failed!, Error:'" . $db_conn->ErrorNo() . "', SQL:\"" . str_replace("\n", "", str_replace("\r", "", str_replace("\t", " ", $sql))) . "'", FALSE);
				return(0);
			}
		}
	}
...
function db_fetch_cell($sql, $col_name = '', $log = TRUE, $db_conn = FALSE) {
	...
	if (read_config_option("log_verbosity") == VERBOSITY_DEVDBG) {
		app_log("DEVEL: SQL Cell: \"" . $sql . "\"", FALSE);
	}
	...
	if (($query) || ($db_conn->ErrorNo() == 1032)) {
		...
		printf("FATAL: Database or Table does not exist");
		exit;
	}else if (($log) || (read_config_option("log_verbosity") == VERBOSITY_DEBUG)) {
		app_log("ERROR: SQL Cell Failed!, Error:'" . $db_conn->ErrorNo() . "', SQL:\"" . str_replace("\n", "", str_replace("\r", "", str_replace("\t", " ", $sql))) . "\"", FALSE);
	}
}
...
function db_fetch_row($sql, $log = TRUE, $db_conn = FALSE) {
	...
	//$sql = str_replace("\n", "", str_replace("\r", "", str_replace("\t", " ", $sql)));
	$sql = str_replace("\r", "", str_replace("\t", " ", $sql));

	if (($log) && (read_config_option("log_verbosity") == VERBOSITY_DEVDBG)) {
		app_log("DEVEL: SQL Row: \"" . $sql . "\"", FALSE);
	}
	...
	if (($query) || ($db_conn->ErrorNo() == 1032)) {
		...
		app_log("ERROR: SQL Row Failed!, Error:'" . $db_conn->ErrorNo() . "', SQL:\"" . str_replace("\n", "", str_replace("\r", "", str_replace("\t", " ", $sql))) . "\"", FALSE);
	}
}
...
function db_fetch_assoc($sql, $log = TRUE, $db_conn = FALSE) {
	...
	if (read_config_option("log_verbosity") == VERBOSITY_DEVDBG) {
		app_log("DEVEL: SQL Assoc: \"" . $sql . "\"", FALSE);
	}
	...
	{
		app_log("ERROR: SQL Assoc Failed!, Error:'" . $db_conn->ErrorNo() . "', SQL:\"" . str_replace("\n", "", str_replace("\r", "", str_replace("\t", " ", $sql))) . "\"");
	}
}
